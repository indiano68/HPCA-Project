...
template <class T>
__device__ inline void merge_small_tiled(const T *v_A_ptr,
                                    const size_t v_A_size,
                                    const T *v_B_ptr,
                                    const size_t v_B_size,
                                    T *v_out_ptr,
                                    const size_t v_out_size,
                                    uint8_t * total_shared_buffer,
                                    bool skip_last)
{
    ...
    for (int n_tile = 0; n_tile < tot_tiles; n_tile++)
    {    
        ...
        __syncthreads();
        Q_shift_buffer = merge_tile(tile_A, tile_B, tile_out, blockDim.x,false);
    ...
    if(current_out_idx < v_out_size)
    {
        ...
        if(current_out_idx +threadIdx.x < v_out_size - (int)skip_last)
            current_out_ptr[threadIdx.x] = tile_out[threadIdx.x];
    }
}
                                    ...
                                    const size_t v_out_size)
{
    ...
    merge_small_tiled(v_A_ptr,v_A_size,v_B_ptr,v_B_size,v_out_ptr,v_out_size,total_shared_buffer,false);
}
